...
class _TechComplaintsScreenState extends State<TechComplaintsScreen>
  ...
  Future<void> _fetchTickets() async {
    ...
    final result = await TechDataService.getComplaints(
  ...
  Future<void> _updateStatus(String complaintId, String newStatus, String? note) async {
    final result = await TechDataService.updateComplaintStatus(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // Arrival OTP Verification Action
                if (ticket['is_otp_verified'] != true && currentStatus != 'resolved' && currentStatus != 'rejected') ...[
                  Container(
                ...
                ] else if (ticket['is_otp_verified'] == true) ...[
                  Container(
                    width: double.infinity,
                    margin: const EdgeInsets.only(bottom: 20),
                  ...
                  children: [
                    if (currentStatus != 'in_progress')
                      Expanded(
                        child: _statusButton(
                          ...
                          () {
                            Navigator.pop(ctx);
                            _updateStatus(ticket['id'].toString(), 'in_progress', noteController.text.trim().isEmpty ? null : noteController.text.trim());
                          },
                        ),
                      ),
                    ...
                    Expanded(
                      child: _statusButton(
                        ...
                        () {
                          Navigator.pop(ctx);
                          _updateStatus(ticket['id'].toString(), 'resolved', noteController.text.trim().isEmpty ? null : noteController.text.trim());
                        },
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _statusButton(
                        ...
                        () {
                          Navigator.pop(ctx);
                          _updateStatus(ticket['id'].toString(), 'rejected', noteController.text.trim().isEmpty ? null : noteController.text.trim());
                        },
                      ),
                    ),
                  ],
  ...
  void _showOtpVerifyDialog(Map<String, dynamic> ticket) {
                ...
                onPressed: isVerifying
                    ...
                    : () async {
                        ...
                        final result = await TechDataService.verifyComplaintOtp(
                          complaintId: ticket['id'].toString(),
                          otp: otp,
                        );
                        if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Support Tickets', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
        bottom: TabBar(
                      ...
                      child: Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Customer tickets will appear here', style: GoogleFonts.inter(fontSize: 13, color: Colors.grey[400])),
                          ],
                        ),
                      ),
                    ),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Row(children: [
                  Icon(_getCategoryIcon(t['category']?.toString() ?? ''), size: 16, color: const Color(0xFF1E3A8A)),
                  const SizedBox(width: 6),
                  Text(t['category']?.toString() ?? '', style: GoogleFonts.inter(fontSize: 11, color: Colors.grey[600], fontWeight: FontWeight.w500)),
                ]),
                Container(
            ...
            if (t['is_otp_verified'] == true) ...[
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.inter(fontSize: 10, color: Colors.green[800], fontWeight: FontWeight.w600),
                    ),
                  ],
                ),
              ),
              ...
              Row(
                children: [
                  const Icon(Icons.lock_clock, size: 13, color: Color(0xFFD97706)),
                  const SizedBox(width: 4),
                  Text(
                    'Arrival OTP Pending',
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFFF9B515).withOpacity(0.2),
                        borderRadius: BorderRadius.circular(6),
                        border: Border.all(color: const Color(0xFFF9B515).withOpacity(0.5)),
                      ),
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          const Icon(Icons.key, size: 11, color: Color(0xFFB45309)),
                          const SizedBox(width: 3),
                          Text(
                            'Verify OTP',
                            style: GoogleFonts.inter(fontSize: 10, color: const Color(0xFFB45309), fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
                    ),
